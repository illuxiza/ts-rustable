...
import { IdxVec } from 'packages/commons/src';

describe('IdxVec', () => {
  let array: IdxVec<number>;

  beforeEach(() => {
    array = IdxVec.new<number>();
  });
  ...
  test('creation using IdxVec() global method', () => {
    const array = [1, 2, 3, 4, 5];
    const vector = new IdxVec(array);

    expect(vector.len()).toBe(5);
    expect(vector[0]).toBe(1);
    expect(vector[4]).toBe(5);

    const emptyVector = new IdxVec([]);
    expect(emptyVector.isEmpty()).toBe(true);

    const stringVector = new IdxVec('hello');
    expect(stringVector.len()).toBe(5);
    expect(stringVector[0]).toBe('h');

    const setVector = new IdxVec(new Set([1, 2, 3]));
  ...
  describe('retain methods', () => {
    test('retain', () => {
      const vec = IdxVec.from([1, 2, 3, 4, 5, 6]);
      vec.retain((x) => x % 2 === 0);
  ...
  describe('drain methods', () => {
    ...
    test('drain with predicate', () => {
      const vec = IdxVec.from([1, 2, 3, 4, 5]);
  ...
  describe('dedup methods', () => {
    ...
    test('dedupByKey', () => {
      const vec = IdxVec.from([10, 20, 21, 30, 20]);
      vec.dedupBy((i) => Math.floor(i / 10));
      expect([...vec]).toEqual([10, 20, 30, 20]);

      const vec2 = IdxVec.from(['apple', 'banana', 'cherry', 'date', 'elderberry']);
      vec2.dedupBy((s) => s.length);
      expect([...vec2]).toEqual(['apple', 'banana', 'date', 'elderberry']);

      const vec3 = IdxVec.from([1.1, 1.2, 2.1, 2.2, 3.1]);
  ...
  describe('sort methods', () => {
    test('sort with default comparison', () => {
      const vec = IdxVec.from([3, 1, 4, 1, 5]);
      vec.sort();
    ...
    test('sortBy', () => {
      const vec = IdxVec.from(['hello', 'a', 'world']);
    ...
    test('isSortedBy', () => {
      const vec = IdxVec.from(['a', 'hello', 'world']);
  ...
  describe('IdxVec advanced operations', () => {
    ...
    test('split operations', () => {
      ...
      const vec2 = IdxVec.from([1, 2, 3, 4, 5]);
