import { Some, None, Option, isSome, isNone } from '../src/option';
...
describe('Option', () => {
    describe('Some', () => {
        ...
        test('should handle map operations', () => {
            ...
            expect(some.mapOrElse(() => 0, x => x * 2)).toBe(84);
        });
    ...
    describe('None', () => {
        test('should handle basic operations', () => {
            ...
            expect(None.isSomeAnd(x => true)).toBe(false);
        });
        ...
        test('should handle map operations', () => {
            expect(None.map(x => 42).isNone()).toBe(true);
            expect(None.mapOr(0, x => 42)).toBe(0);
            expect(None.mapOrElse(() => 0, x => 42)).toBe(0);
        });

        test('should handle andThen', () => {
            expect(None.andThen(x => Some(42)).isNone()).toBe(true);
        });
        ...
        test('should handle filter', () => {
            expect(None.filter(x => true).isNone()).toBe(true);
        });
    ...
    describe('Complex Types', () => {
        test('should handle objects', () => {
            ...
            expect(some.map(o => ({ ...o, z: 3 })).unwrap()).toEqual({ x: 1, y: 2, z: 3 });
        });
